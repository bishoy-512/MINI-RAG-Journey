...
async def process_endpoint(request: Request,project_id : str , process_request : ProcessRequests):
    ...
    if process_request.file_id:
        ...
        if asset_record is None:
            return JSONResponse(
                ...
                content={"Response":ResponseEnum.FILE_ID_ERROR.value}
            )
        ...
        if len(project_files_ids) == 0:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={"Response":ResponseEnum.NO_FILE_ERROR.value}
            )
    ...
    no_records = 0
    no_files = 0
    for _id , file_id in project_files_ids.items():
