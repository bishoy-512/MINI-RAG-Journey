...
class QdrantDB(VectorDBInterface):
    ...
    def insert_one(self, collection_name: str, text: str, vector: list,metadata: dict = None, 
        ...
        try:
            _ = self.client.upload_points(collection_name = collection_name,
                records = 
                    [
                        models.PointStruct(
                            id=[record_id],
    ...
    def insert_many(self, collection_name, texts, vectors, metadata = None, record_ids = None, batch_size = 50):
        ...
        for i in range(0 , len(texts) , batch_size):
            ...
            try:
                _ = self.client.upload_points(collection_name=collection_name,points=batch_records)
            except Exception as e:
                self.logger.error(f"Error while inserting batch: {e}")
                return False
    ...
    def search_by_vector(self, collection_name: str, vector: list, limit: int = 5):
        return self.client.query_points(collection_name=collection_name,query=vector,limit=limit).points